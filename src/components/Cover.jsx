...
function Cover({fetchUrl}) {
  ...
  return (
   <div style={{height:'560px', backgroundImage:`url(${base_url}${movie.backdrop_path})`,backgroundSize:'cover',backgroundAttachment:'fixed'}}>
    <div className="banner-details">
      <h1>{movie.name}</h1>
      <button className='btn btn-danger'>Play</button>
      <button className='btn btn-outline-light ms-4'>More Info</button>
      <h5>{movie.overview?.slice(0,200)}...</h5>
    </div>
   </div>
  )
}
